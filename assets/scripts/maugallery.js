  ...
  $.fn.mauGallery.methods = {
    ...
    wrapItemInColumn($element, columns) {
      ...
      if (typeof columns === 'number') {
        ...
        console.error(`Type des colonnes invalide : ${typeof columns}`);
        return;
      }
    ...
    navigateImage(lightboxId, direction) {
      ...
      let $images = activeTag === "all" 
        ? $(".item-column img") 
        : $(".item-column img").filter((_, img) => $(img).data("gallery-tag") === activeTag);
      
      let index = $images.index($activeImage);
    ...
    showItemTags($gallery, position, tags) {
      const tagItems = tags.reduce((acc, tag) => {
        acc += `<li class="nav-item"><span class="nav-link" data-images-toggle="${tag}">${tag}</span></li>`;
        return acc;
      }, `<li class="nav-item"><span class="nav-link active active-tag" data-images-toggle="all">Tous</span></li>`);

      const tagsRow = `<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;
      position === "bottom" ? $gallery.append(tagsRow) : position === "top" ? $gallery.prepend(tagsRow) : console.error(`Position des tags inconnue : ${position}`);
    },
    ...
    filterByTag() {
      ...
      $(".gallery-item").each(function() {
        ...
        tag === "all" || $(this).data("gallery-tag") === tag ? $itemColumn.show(300) : $itemColumn.hide();
      });
    }
  };
